...
void StatReader::PrintBus(const TransportCatalogue &tc,
  ...
  auto uniqCalc = [](const std::vector<const Stop *> &stops) -> size_t {
    ...
    std::for_each(
        ...
        [&uniqueStops](const Stop *stop) { uniqueStops.insert(stop->name); });
  ...
  while (it != (stops.cend() - 1)) {
    ...
    directDistance +=
        ComputeDistance(tc.GetStopCoords(nameA), tc.GetStopCoords(nameB));
  ...
  //        Bus 256: 6 stops on route, 5 unique stops, 5950 route
  //        length, 1.36124 curvature
  os_ << "Bus " << name << ": " << stops.size() << " stops on route, "
      << uniqCalc(stops) << " unique stops, " << realDistance
      << " route length, " << std::setprecision(6)
      << realDistance / directDistance << " curvature" << std::endl;
}

void StatReader::PrintStop(const TransportCatalogue &tc, std::string_view name) const {
  // std::unordered_map<std::string_view, std::pair<Stop*,
  // BusPointersContainer>> stopnameToStop_{};
